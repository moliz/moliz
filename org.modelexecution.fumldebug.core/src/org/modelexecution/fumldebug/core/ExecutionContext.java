...
public class ExecutionContext implements ExecutionEventProvider {
	...
	/*
	 * Data structure for storing executions to their IDs The executions started
	 * by the user (through call of execute(...) or debug(...) remain in this
	 * data structure in this execution context Executions called by such
	 * executions are deleted if their execution ended.
	 */
	...
	/**
	 * Performs the next execution step in the activity execution with the given
	 * ID by executing the provided node
	 * 
	 * @param executionID
	 *            ID of the activity execution for which the next step shall be
	 *            performed
	 * @param node
	 *            activity node which shall be executed in the next step
	...
	public void nextStep(int executionID, ActivityNode node)
		...
		ActivityExecution activityExecution = activityExecutions
				.get(executionID);
		if (node == null) {
			nextnode = getNextNode(activityExecution);
			activityExecution = activityExecutions.get(nextnode
					.getExecutionID());
		} else {
			nextnode = new ActivityNodeChoice(executionID, node);
		}

		ExecutionStatus exestatus = activityExecutionStatus
				.get(activityExecution);
		boolean activityNodeWasEnabled = exestatus.getEnabledNodes().remove(
				nextnode.getActivityNode());
...
//		}	
	...
	private ActivityNodeChoice getNextNode(ActivityExecution activityExecution)
			throws IllegalArgumentException {
		...
		ActivityNodeChoice nextnode = this.nextNodeStrategy.chooseNextNode(
				activityExecution, this.executionhierarchy,
	 ...
	 * 
	 * @param executionID
	 *            ID of the activity execution which shall be resumed
	 * @throws IllegalArgumentException
	...
	protected Trace getTrace(ActivityExecution execution) {
		...
		ActivityExecution rootExecution = executionhierarchy
				.getRootCaller(execution);
	 ...
	 * 
	 * @param activitynode
	 *            ActivityNode for which shall be checked if a breakpoint is set
	 * @return breakpoint that is set for the given ActivityNode, null if no
	 *         breakpoint is set
	 */
	 ...
	 * 
	 * @param activitynode
	 *            ActivityNode for which a set breakpoint shall be removed
	 */
	...
	protected void setExecutionInResumeMode(ActivityExecution execution,
			boolean resume) {
		ActivityExecution caller = this.executionhierarchy.getCaller(execution);
		if (caller != null) {
	...
	protected void removeActivityExecution(ActivityExecution execution) {
		List<ActivityExecution> callees = executionhierarchy
				.getCallee(execution);
	...
	/**
	 * Terminates the execution of an activity. If the executionID of an called
	 * activity execution (e.g., CallBehaviorAction) is provided, the whole
	 * activity execution including the root activity execution and all called
	 * executions are terminated as well.
	 * 
	 * @param executionID
	 *            of the activity execution that shall be terminated.
	 */
	...
	protected void addActivityExecution(ActivityExecution execution,
			ActivityNodeActivation caller, ActivityEntryEvent entryevent) {
	 ...
	 * @return
	 */
	protected ExecutionStatus getActivityExecutionStatus(
			ActivityExecution execution) {
		return activityExecutionStatus.get(execution);
	}

	/**
	 * Determines if the given activity execution has enabled nodes including
	 * called activities
	 * 
	 * @param execution
	 * @return
	 */
	protected boolean hasEnabledNodesIncludingCallees(
			ActivityExecution execution) {
		ExecutionStatus executionstatus = activityExecutionStatus
				.get(execution);
		...
		if (callees != null) {
			for (int i = 0; i < callees.size(); ++i) {
				boolean hasenablednodes = hasEnabledNodesIncludingCallees(callees
						.get(i));
				if (hasenablednodes) {
					return true;
				}
	...
	/**
	 * Determines if the caller of the given activity execution has enabled
	 * nodes
	 * 
	 * @param execution
	 * @return
	...
	protected void setActivityExecutionOutput(ActivityExecution execution,
...
/*TODO		
		...
		org.modelexecution.fumldebug.core.trace.tracemodel.ActivityExecution activityExecutionTrace = trace
			...
			for(OutputPin outputPin : action.output) {					
				...
				if (sentTokens != null) {
					for (Token token : sentTokens) {
						TokenInstance tokenInstance = executionStatus
						...
						if (tokenInstance == null) {
							...
							if (token.getValue() instanceof Reference) {
								valueInstance.setValue(((Reference) token
										.getValue()).referent.copy());
							} else {
								if (token.getValue() != null) {
									valueInstance.setValue(token.getValue()
											.copy());
								}
							}
							otokenInstance.setValue(valueInstance);
							executionStatus.addTokenInstance(token,
									otokenInstance);
							tokenInstances.add(otokenInstance);
						}
					}
					if (tokenInstances.size() > 0) {
						nodeExecution.addActivityNodeOutput(outputPin,
								tokenInstances);
					}
				}
			}
			...
			for (Token token : s) {
				...
				if (t instanceof ControlToken) {
					...
					if (t.getValue() instanceof Reference) {
						valueInstance
								.setValue(((Reference) t.getValue()).referent
										.copy());
					} else {
						valueInstance.setValue(t.getValue().copy());
					}
					((ObjectTokenInstance) tokenInstance)
...
/* TODO				
			...
			if (activation instanceof ActionActivation) {
				...
				for (InputPin inputPin : action.input) {
					PinActivation inputPinActivation = actionActivation
							.getPinActivation(inputPin);
					TokenList heldtokens = inputPinActivation.heldTokens;

					List<TokenInstance> tokenInstances = new ArrayList<TokenInstance>();
					for (Token token : heldtokens) {
						Token originalToken = executionStatus
								.getOriginalToken(token);
						if (tokens.contains(originalToken)) {
							TokenInstance tokenInstance = executionStatus
									.getTokenInstance(originalToken);
							if (tokenInstance != null) {
								List<ActivityEdge> traversedEdges = executionStatus
										.getTraversedActivityEdges(originalToken);
								List<ActivityEdge> traversedEdgesForNode = getTraversedEdge(
										traversedEdges, inputPinActivation.node);
								tokenInstance.getTraversedEdges().addAll(
										traversedEdgesForNode);
								tokenInstances.add(tokenInstance);
							}
...
/* TODO			
			...
			if (activation instanceof DecisionNodeActivation) {
				...
				ActivityEdgeInstance decisionInputFlowInstance = decisionNodeActivation
			...
			for (Token token : tokens) {
				...
				if (tokenInstance != null) {
					List<ActivityEdge> traversedEdges = executionStatus
							.getTraversedActivityEdges(token);
					List<ActivityEdge> traversedEdgesForNode = getTraversedEdge(
							traversedEdges, node);
